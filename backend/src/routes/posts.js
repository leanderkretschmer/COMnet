...
router.put('/:postId', authenticateToken, async (req, res) => {
  try {
    ...
    // Check if post exists and user is author or community admin
    const postResult = await query(
      `SELECT p.author_id, c.name as community_name, cm.role
       FROM posts p
       JOIN communities c ON p.community_id = c.id
       LEFT JOIN community_members cm ON c.id = cm.community_id AND cm.user_id = $1
       WHERE p.id = $2 AND p.network_id = $3`,
      [req.user.id, postId, req.networkId]
    );
    ...
    const isModerator = post.role === 'moderator' || post.role === 'admin';

    if (!isAuthor && !isModerator) {
      return res.status(403).json({ error: 'Keine Berechtigung zum Bearbeiten dieses Posts' });
    }
    ...
    const result = await query(
      `UPDATE posts 
       ...
       RETURNING id, title, content, is_nsfw, updated_at`,
      updateValues
    );
...
router.delete('/:postId', authenticateToken, async (req, res) => {
  try {
    ...
    if (!isAuthor && !isModerator) {
      return res.status(403).json({ error: 'Keine Berechtigung zum Löschen dieses Posts' });
    }
